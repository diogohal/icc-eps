...
matriz_t *criaMatriz(int tam) {
    ...
    for(int i=0; i<tam; i++) {
        ...
        transformaIntervalo(&matriz->residuo[i], 0);
    }
...
void recebeEntrada(matriz_t *matriz) {
    ...
    for(int i=0; i<matriz->tam; i++) {
        for(int j=0; j<matriz->tam; j++) {
            returnScanf = scanf("%lf", &aux);
            transformaIntervalo(&matriz->A[i][j], aux);
        }
        returnScanf = scanf("%lf", &aux);
        transformaIntervalo(&matriz->B[i], aux);
    }
}
